...
function itemHTML(name, seller, price, count) {
    ...
    <div class="itemCountWrap">
        <input type="number" name="itemCount" class="itemCount" min="1" value="${count}" onchange="changeEachPrice(${itemNumber})">
    </div>
    <div class="itemTotalPriceWrap">
        <span class="itemTotalPrice" value="${price * count}">${Number(price*count).toLocaleString()}</span>원
    </div>
    <button onclick="removeItem(${itemNumber})"><i class="bi bi-x-lg"></i></button>
</li>`;
    itemNumber++;
    return returnHTML;
}
...
function changeEachPrice(itemId) {
    ...
    $(`#item${itemId} .itemTotalPrice`).text(Number(totalPrice).toLocaleString());
...
function couponChange() {
    ...
    let basketPrice = Number($(".itemTotalPriceSumWrap .itemTotalPriceSum").text().replaceAll(",",""));
